...
app.put("/sampleWords", function(request, response) {
  mongodb.collection("sampleWords").insertOne( {
...
app.get("/sampleWords", function(request, response) {
  // and we call on the connection to return us all the documents in the
  // words collection.
  mongodb.collection("sampleWords").find().toArray(function(err, sampleWords) {
    if (err) {
     response.status(500).send(err);
    } else {
     response.send(sampleWords);
    }
  });
});

app.get("/starredWords", function(request, response) {
  // and we call on the connection to return us all the documents in the
  // words collection.
  mongodb.collection("sampleWords").find({"starred": true}).toArray(function(err, sampleWords) {
    if (err) {
     response.status(500).send(err);
    } else {
     response.send(sampleWords);
...
app.get("/singleSearch", function(request, response) {
  // and we call on the connection to return us all the documents in the
  // words collection.
  if(!request.query.term){
    response.status(400).send("no term specified");
    return;
  }
  mongodb.collection("sampleWords").find({"term": RegExp('^' + request.query.term + '$', 'i')}).toArray(function(err, sampleWords) {
    if (err) {
     response.status(500).send(err);
    } else {
     response.send(sampleWords);
...
app.get("/certainTag", function(request, response) {
  // and we call on the connection to return us all the documents in the
  // words collection.
  ...
  mongodb.collection("sampleWords").find({"tags": request.query.tags}).toArray(function(err, sampleWords) {
    if (err) {
     response.status(500).send(err);
    } else {
     response.send(sampleWords);
...
app.post("/changeStar", function(request, response){
  if(request.query.starred){
    mongodb.collection("sampleWords").update({"term": request.query.term}, {starred: "false"});
    response.send(sampleWords);
  }else{
    mongodb.collection("sampleWords").update({"term": request.query.term}, {starred: "true"});
    respone.send(sampleWords);
  }
});
